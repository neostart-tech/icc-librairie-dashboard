<template>
	...
	<div class="min-h-screen p-6 bg-gray-50 dark:bg-gray-900 space-y-6">
		...
		>
			<div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all border border-gray-100 dark:border-gray-700">
				
				<!-- Modal Header with gradient -->
				<div class="relative p-6 bg-gradient-to-r from-[#6a0d5f] to-[#9a1b8c] text-white">
					<div class="absolute top-0 right-0 p-4">
						<button
							v-if="!isImporting || finishedImport"
					...
					<div class="flex items-center gap-4">
						...
						<div>
							<h3 class="text-xl font-bold tracking-tight">Importation Excel</h3>
							<p class="text-white/80 text-sm font-medium mt-0.5">Ajout de livres en masse</p>
						</div>
					</div>
				</div>
				...
				<div class="p-8">
					...
					<div class="flex flex-col items-center justify-center text-center mb-8">
						<div v-if="finishedImport" class="w-16 h-16 bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-full flex items-center justify-center mb-4 shadow-sm transform scale-100 animate-[bounce_0.5s_ease-out]">
							<svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" />
							</svg>
						</div>
						<div v-else-if="cancelImport" class="w-16 h-16 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-full flex items-center justify-center mb-4 shadow-sm">
							<svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12" />
							</svg>
						</div>
						<div v-else class="w-16 h-16 bg-blue-50 dark:bg-blue-900/20 text-[#6a0d5f] dark:text-purple-400 rounded-full flex items-center justify-center mb-4 shadow-sm relative">
							<svg class="w-7 h-7 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
							</svg>
							<!-- Circular spinner -->
							<svg class="absolute inset-0 w-full h-full text-[#6a0d5f]/20 dark:text-purple-400/20 animate-spin" viewBox="0 0 24 24">
						...
						<h4 class="text-lg font-bold text-gray-900 dark:text-white">
							{{ finishedImport ? 'Opération réussie !' : (cancelImport ? 'Importation annulée' : 'Traitement en cours...') }}
						</h4>
						<p class="text-gray-500 dark:text-gray-400 mt-1.5 text-sm leading-relaxed">
							{{ finishedImport ? 'Vos livres ont été importés avec succès dans le système.' : (cancelImport ? 'Le processus d\'importation a été interrompu par l\'utilisateur.' : 'Veuillez ne pas fermer cette fenêtre pendant l\'importation des données.') }}
						</p>
					</div>
					
					<!-- Progress Bar Section -->
					<div class="bg-gray-50 dark:bg-gray-800/80 rounded-2xl p-5 border border-gray-100 dark:border-gray-700/50 mb-7 shadow-sm">
						<div class="flex justify-between text-sm font-semibold text-gray-700 dark:text-gray-200 mb-3">
							<span class="flex items-center gap-2">
								<span v-if="!finishedImport && !cancelImport" class="relative flex h-2.5 w-2.5">
								  <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#6a0d5f] opacity-75"></span>
						...
						<div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3 mb-4 overflow-hidden shadow-inner">
							<div 
								class="h-3 rounded-full transition-all duration-300 ease-out relative overflow-hidden" 
								:class="finishedImport ? 'bg-green-500' : (cancelImport ? 'bg-red-500' : 'bg-gradient-to-r from-[#6a0d5f] to-[#e43b8a]')"
						...
						<div class="flex justify-between items-center text-xs">
							...
							<div class="px-2.5 py-1.5 bg-white dark:bg-gray-900 rounded-lg font-bold text-gray-700 dark:text-gray-300 shadow-sm border border-gray-100 dark:border-gray-700 flex items-center gap-1.5">
								<span class="text-[#6a0d5f] dark:text-purple-400">{{ importCurrent }}</span> 
					...
					<div class="flex justify-center gap-3">
						<button
							v-if="isImporting && !finishedImport && !cancelImport"
						...
						<button
							v-if="finishedImport || cancelImport"
							@click="closeImportModal"
						...
						>
							<svg v-if="finishedImport" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
							Fermer la fenêtre
						</button>
					</div>
				</div>
		...
		reader.onload = async (event: any) => {
			try {
				...
				for (let i = 0; i < rows.length; i++) {
					...
					importProgress.value = Math.round(((i + 1) / Math.max(1, rows.length)) * 100);
